...
/**
 * search_blk - Searches for a value in a part of a list of integers
 * @list: Is a pointer to the head of the list to search in
...
listint_t *search_blk(listint_t *list, size_t end, int value)
{
	listint_t *result = NULL;
...
listint_t *jump_list(listint_t *list, size_t size, int value)
{
	...
	for (block_start = 0; block_start < size; block_start += jump_step)
	{
		...
		if (block_end < size)
		{
			if (cur->n >= value)
			{
				result = search_blk(prev, block_last_index, value);
				break;
			}
		}
		else
